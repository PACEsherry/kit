...
FFI_EXPORT int64_t FfiCameraManagerCreatePreviewOutput(CJProfile profile, const char *surfaceId, int32_t *errCode, char **errMessage)
{
    std::string surfaceId_s = std::string(surfaceId);
    Size size = {profile.width, profile.height};
    Profile c_Profile(CameraFormat(profile.format), size);
    ...
    *errCode = CJPreviewOutput::CreatePreviewOutput(c_Profile, surfaceId_s, errMsg);
    if (*errCode == CameraError::CAMERA_SERVICE_ERROR) {
        if (errMessage != nullptr) {
            *errMessage = MallocCString(errMsg);
        }
        return -1;
    }
    ...
    if (previewOutputImpl == nullptr) {
        *errCode = CameraError::CAMERA_SERVICE_ERROR;
        if (errMessage != nullptr) {
            *errMessage = MallocCString("Failed to create previewOutput instance");
        }
        return -1;
    }
    *errCode = CameraError::NO_ERROR;
