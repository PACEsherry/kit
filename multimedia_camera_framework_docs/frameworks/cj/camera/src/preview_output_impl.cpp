...
int32_t CJPreviewOutput::CreatePreviewOutput(Profile &profile, std::string &surfaceId, std::string &errMsg)
{
    uint64_t iSurfaceId;
    std::istringstream iss(surfaceId);
    iss >> iSurfaceId;
    ...
    if (surface == nullptr) {
        std::string errorMsg = "Failed to get previewOutput surface";
        MEDIA_ERR_LOG("%s", errorMsg.c_str());
        errMsg = errorMsg;
        return CameraError::CAMERA_SERVICE_ERROR;
    }
    surface->SetUserData(CameraManager::surfaceFormat, std::to_string(profile.GetCameraFormat()));
    int retCode = CameraManager::GetInstance()->CreatePreviewOutput(profile, surface, &sPreviewOutput_);
    if (sPreviewOutput_ == nullptr) {
        std::string errorMsg = "failed to create previewOutput";
        MEDIA_ERR_LOG("%s", errorMsg.c_str());
        errMsg = errorMsg;
        return CameraError::CAMERA_SERVICE_ERROR;
    }
